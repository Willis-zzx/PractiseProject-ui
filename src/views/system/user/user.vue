...
export default {
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: 'localhost:8081' + '/system/user/importData'
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  components: {
    Pagination,
    Treeselect // 不能命名为TreeSelect，下拉树会不生效
  },
  watch: {
    // 根据名称筛选部门树
    deptName(val) {
  ...
  methods: {
    ...
    getList() {
      ...
      listUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
        ...
        if (response.success === false) {
          ...
          // 分页赋值
          this.pageParams.currentPage = this.queryParams.pageNum
          this.pageParams.pageSize = this.queryParams.pageSize
    ...
    /** 分页插件事件 */
    callFather(param) {
      this.queryParams.pageNum = param.currentPage
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const userIds = row.user || this.ids
